...
function changeModule(module){
	...
	switch(module){
		...
		default:
			...
			if (moduleExistsInArray(module)) {
				$("#module_quiz").empty(); // Remove the quiz link element
			} else {
...
function changeQuiz(module){
	...
	if(module == 1 || (module > 1 && (quizzes[module-1] >= 100 || quizzes[module-1] == -2))){
...
function moduleExistsInArray(module) {
    // Return true if the module exists, otherwise return false
    var modulesArray = '#X#'
    return modulesArray.includes(module);
}
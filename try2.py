...
class OCPWizApp(ctk.CTk):
    def __init__(self, *args, **kwargs):
        ...
        img = Image.open(os.path.join(os.path.dirname(__file__),'template/course/img/upou.png'))
        ...
        self.resources_btn = ctk.CTkButton(sidebar_frame,text='',width=165,height=70,font=LARGEFONT,anchor="w",state=ctk.DISABLED,fg_color=up_maroon,hover_color=up_maroon_d,
                                           command=lambda: self.indicate(self.resources_btn, ResourcesPage,))
    ...
    def create_ocp(self):
        ...
        self.copy_file2(os.path.join(os.path.dirname(__file__),f'{curr_dir}/template/JS/course.js'), 
        ...
        self.copy_file2(os.path.join(os.path.dirname(__file__),f'{curr_dir}/template/JS/register.js'), 
        ...
        self.copy_file(os.path.join(os.path.dirname(__file__),f'{curr_dir}/template/JS/progress.js'), 
        ...
        #CSS FILES ---------------------------------------------------------------------------------------------------------------------
        self.copy_file(os.path.join(os.path.dirname(__file__),f'{curr_dir}/template/CSS/course.css'), 
...
class ResourcesPage(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        resources_page=ctk.CTkFrame(self)
        resources_page.pack(anchor= ctk.CENTER, fill='both', expand=True)
        resources_page.grid_columnconfigure(0, weight=1)
        resources_page.grid_columnconfigure(2, weight=1)

        self.course_guide_btn = ctk.CTkButton(resources_page,text='Upload Course Guide', font=LARGEFONT,width=410,height=40,
                        command=lambda value='Course Guide': self.save_dirs(value))
        self.course_guide_btn.grid(row=1,column=1,sticky=tk.W,pady=20)

        resources_btn = ctk.CTkButton(resources_page,text='Add Folders', font=LARGEFONT,width=410,height=40,
                        command=lambda value='Folder': self.save_dirs(value))
        resources_btn.grid(row=2,column=1,sticky=tk.W,pady=20)

        files_btn = ctk.CTkButton(resources_page,text='Add Files', font=LARGEFONT,width=410,height=40,
                        command=lambda value='File': self.save_dirs(value))
        files_btn.grid(row=3,column=1,sticky=tk.W,pady=20)

        files_btn = ctk.CTkButton(resources_page,text='Remove Resources', font=LARGEFONT,width=410,height=40,
...
class CreateQuizPage(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        self.create_quiz_page.grid_columnconfigure(2, weight=1)
        ...
        webopen_btn=ctk.CTkButton(footer, text='What is GIFT format?', font=LARGEFONT,height=50,width=215,
                               command= lambda: webbrowser.open(os.path.join(os.path.dirname(__file__),'gift.txt')))
    ...
    def update_btns(self):
        ...
        for k in questions.keys():
            ...
            if k != 'final':
                ...
                btn = ctk.CTkButton(self.create_quiz_page,text=btn_text, font=LARGEFONT,width=410,height=40,
                            command=lambda value=k: [self.create_quiz(value)])
                btn.grid(row=len(questions),column=1,sticky=tk.W,pady=20)
